...
ROBOT = "e3_21dof"
INPUT_SCENE_PATH = "resources/robots/e3_21dof/scene.xml"
OUTPUT_SCENE_PATH = "resources/robots/e3_21dof/scene_terrain.xml"
...
class TerrainGenerator:
    ...
    def AddPlatform(self,
        ...
        """
        x, y = rot2d(0, 0, yaw)
        position = [init_pos[0] + x, init_pos[1] + y, init_pos[2] + height / 2.0]
        self.AddBox(position, [0.0, 0.0, yaw], [length, width, height])
    ...
    def AddGap(self,
        ...
        Args:
            ...
            gap_length: gap长度（米）
            platform_length: 每个平台的长度（米）
        """
        # 第一个平台
        pos1 = [init_pos[0], init_pos[1], init_pos[2]]
        self.AddPlatform(pos1, yaw, width, platform_length, platform_height)
        
        # 第二个平台（在gap之后）
        # 计算第二个平台的位置
        gap_center_x = platform_length / 2.0 + gap_length / 2.0
        gap_center_y = 0.0
        x, y = rot2d(gap_center_x, gap_center_y, yaw)
        pos2 = [init_pos[0] + x, init_pos[1] + y, init_pos[2]]
        self.AddPlatform(pos2, yaw, width, platform_length, platform_height)
...
if __name__ == "__main__":
    ...
    # Slope
    # tg.AddBox(position=[2.0, 0.0, 0.2],
    #           euler=[0.0, -0.2, 0.0],
    #           size=[3, 1.5, 0.1])

    # Stairs
    tg.AddStairs(init_pos=[1.0, 0.0, 0.0], yaw=0.0, height=0.16)

    # Suspend stairs
    # tg.AddSuspendStairs(init_pos=[1.0, 6.0, 0.0], yaw=0.0)

    
    # 高度为30cm和40cm的宽为2m的高台
    tg.AddPlatform(init_pos=[-1.0, 0.0, 0.0], yaw=0.0, width=2.0, length=1.0, height=0.3)
    tg.AddPlatform(init_pos=[-2.4, 0.0, 0.0], yaw=0.0, width=2.0, length=1.0, height=0.3)
    tg.AddPlatform(init_pos=[-5, 0.0, 0.0], yaw=0.0, width=2.0, length=1.0, height=0.3)
