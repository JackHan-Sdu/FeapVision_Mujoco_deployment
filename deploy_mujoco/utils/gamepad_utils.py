...
import json
import os
from .mode_utils import CMD_MAX
...
def get_calibration_file_path(gamepad_type=None, custom_path=None):
    ...
    Returns:
        校准文件路径
    """
    # deploy_mujoco/ 与 vision_deploy/（校准 JSON 默认放在后者）
    deploy_dir = os.path.dirname(os.path.dirname(__file__))
    cal_dir = os.path.join(deploy_dir, "vision_deploy")

    if custom_path:
        if not os.path.isabs(custom_path):
            p_vis = os.path.join(cal_dir, custom_path)
            if os.path.isfile(p_vis):
                return p_vis
    ...
    if gamepad_type:
        ...
        p_vis = os.path.join(cal_dir, base_name)
        if os.path.isfile(p_vis):
            return p_vis
...
def load_gamepad_calibration(config=None):
    ...
    if config is not None:
        ...
        gamepad_type_raw = config.get('gamepad_type')
        # 确保 gamepad_type 是字符串且不为空
        gamepad_type = None
        if gamepad_type_raw:
            gamepad_type = str(gamepad_type_raw).strip().lower()
            if not gamepad_type:
                gamepad_type = None
        axis_mapping_custom = config.get('axis_mapping')
        
        # 调试信息
        if config.get('gamepad_type'):
            print(f"调试: 配置文件中读取到 gamepad_type = {repr(gamepad_type_raw)}, 处理后 = {repr(gamepad_type)}")
        ...
        if gamepad_type and not calibration_file_path:
            ...
            if os.path.exists(calibration_file):
                try:
                    ...
                    print(f"  手柄名称: {gamepad_calibration.get('joystick_name', 'Unknown')}")
                    print(f"  校准日期: {gamepad_calibration.get('calibration_date', 'Unknown')}")
                    loaded_gamepad_type = gamepad_calibration.get('gamepad_type', 'logitech')
                    if loaded_gamepad_type != gamepad_type:
                        print(f"警告: 校准文件中的手柄类型 ({loaded_gamepad_type}) 与配置文件中指定的类型 ({gamepad_type}) 不匹配")
                    print(f"  手柄类型: {loaded_gamepad_type} - 轴映射: 左摇杆({axis_mapping[0]},{axis_mapping[1]}), 右摇杆({axis_mapping[2]},{axis_mapping[3]})")
                    return gamepad_calibration
                except Exception as e:
                    print(f"警告: 加载校准文件失败: {e}")
                    gamepad_calibration = None
                ...
                gamepad_calibration = None
            # 如果配置文件中指定了gamepad_type，无论加载成功与否，都应该返回（不继续尝试其他类型）
            return gamepad_calibration
    
    # 2. 如果没有配置文件或配置文件中没有指定gamepad_type，尝试从校准文件读取（按优先级尝试）
    # 注意：如果配置文件中指定了gamepad_type，上面的代码应该已经返回了，这里不应该执行
    if gamepad_type is None:
        # 按优先级尝试加载：betop > logitech > custom
        for gp_type in ['logitech', 'betop', 'custom']:
...
def print_gamepad_controls():
    """打印游戏手柄控制说明"""
    print("=" * 50)
    print("游戏手柄控制说明 (vision deploy):")
    print("  右摇杆上下: 前后速度 cmd[0]")
    print("  右摇杆左右: 左右速度 cmd[1]")
    print("  左摇杆左右: 角速度 cmd[2]")
    print("  方向键左右: 围绕机器人旋转视角（跟踪 pelvis 时）")
    print("  方向键上下: 视角远近（跟踪 pelvis 时）")
    print("  X: 切换足底力显示 | Y: 跟踪 pelvis | B: 重置 | A: 接触点显示")
    print("=" * 50)
    ...
    if y_button_id is not None and joystick.get_numbuttons() > y_button_id:
        ...
        if button_y_pressed and not button_states.get('y', False):
            ...
            if viewer_utils.track_pelvis:
                ...
                viewer_utils.camera_elevation = -0.2
...
def update_cmd_from_gamepad(cmd):
    """从游戏手柄读取输入并更新速度命令（单档速度限制，见 mode_utils.CMD_MAX）。"""
    if joystick is None:
        return cmd
    ...
    cmd[2] = left_stick_x * cmd_max[3]  # yaw rate
